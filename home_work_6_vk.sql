...
CALL get_random_users(3);


-- Создать функцию, вычисляющей коэффициент популярности пользователя (по количеству друзей)
DROP FUNCTION IF EXISTS popularity_count;
DELIMITER $$
CREATE FUNCTION popularity_count(id BIGINT)
RETURNS BIGINT
DETERMINISTIC
BEGIN 
	DECLARE petcentege_of_popularity INT;
    SELECT m.body INTO petcentege_of_popularity
    FROM messages m
    WHERE m.to_user_id = id
    LIMIT 1;
    RETURN petcentege_of_popularity;

END $$ 
DELIMITER ;

SELECT popularity_count(2);

SELECT 
	COUNT (fr.initiator_user_id) + COUNT (fr.target_user_id)
